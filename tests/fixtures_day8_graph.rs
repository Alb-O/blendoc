...
use blendoc::blend::{BlendFile, GraphOptions, RefScanOptions, build_graph_from_ptr, scan_id_blocks};
...
fn assert_scene_graph(name: &str) {
	...
	let ids = blendoc::blend::IdIndex::build(scan_id_blocks(&blend, &dna).expect("id scan succeeds"));
